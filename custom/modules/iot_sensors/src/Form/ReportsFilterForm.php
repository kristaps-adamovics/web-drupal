...
class ReportsFilterForm extends FormBase {
  ...
  public function getFormId() {
    return 'iot_sensors_reports_filter_form';
  }

  public function buildForm(array $form, FormStateInterface $form_state, array $filters = []) {
    // GET form is used here so filters are visible in the URL.
    $form['#method'] = 'get';
    $form['#action'] = Url::fromRoute('iot_sensors.reports')->toString();
    $form['#attributes']['class'][] = 'iot-report-filters';
    $form['#token'] = FALSE;
    $form['#cache'] = [
    ...
    $form['metric'] = [
      '#type' => 'select',
      '#title' => $this->t('Rādītājs'),
      '#options' => ['' => $this->t('- Visi rādītāji -')] + $this->metricOptions(),
      '#default_value' => (string) ($filters['metric'] ?? ''),
    ];
    $form['sensor_id'] = [
      '#type' => 'select',
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach (['date_from', 'date_to', 'room_id', 'metric', 'sensor_id'] as $key) {
    ...
    $form_state->setRedirect('iot_sensors.reports', [], ['query' => $query]);
  }
  ...
  private function loadSensorOptions() {
    ...
    foreach ($query->execute()->fetchAll() as $sensor) {
      $metric = $this->metricOptions()[$sensor->metric] ?? $sensor->metric;
      $options[$sensor->id] = $this->t('@room: @code (@metric)', [
